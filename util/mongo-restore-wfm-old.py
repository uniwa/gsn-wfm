...
# restore process
#findProcess = subprocess.Popen("find -L /srv/htdocs/wfm2/util/dnikoudis -maxdepth 4 -iname 'my files' -not -empty", stdout=subprocess.PIPE, stderr=subprocess.PIPE, shell=True)
findProcess = subprocess.Popen("find -L /users/2/s/limperis -maxdepth 4 -iname 'my files' -not -empty", stdout=subprocess.PIPE, stderr=subprocess.PIPE, shell=True)
#findProcess = subprocess.Popen("/usr/bin/find -L /users -maxdepth 4 -mindepth 4 -iname 'my files' -not -empty", stdout=subprocess.PIPE, stderr=subprocess.PIPE, shell=True)
for origPath in findProcess.stdout.read().split("\n"):
	if origPath == '':
		break
	path = origPath.split('/')
	...
	restore_folder = "wfm-old-backup-" + now.strftime('%Y%m%d%H%M')
	...
	# restore files
	mongo_restore(user, origPath, restore_id)
	print 'Restored '+user
