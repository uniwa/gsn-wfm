...
import urllib
import urllib2
#for more info see MultipartPostHandler.py file
...
def execute_command(f):
    ...
    request or None if no data is required.
    """
    def wrapper(self, *args, **kwargs):
        ...
        try:
            ...
            return {'success': False, 'status_msg': e}
        # json 2 python dict
...
class WfmApi(object):
    ...
    def cas_login(self):
        ...
        self._file_obj, self._url_obj = caslib.login_to_cas_service(
                                                ...
                                                self._username,
                                                self._password)
        # add handler for uploading files (used by cmd_create_file)
    ...
    def cas_renew(self):
        ...
        self._file_obj, self._url_obj = caslib.login_to_cas_service(
                                                ...
                                                self._username,
                                                self._password)
        # add handler for uploading files requests (used by cmd_create_file)
        self._url_obj.add_handler(MultipartPostHandler.MultipartPostHandler())
    ...
    def wfm_init_calls(self):
        ...
        try:
            ...
            self._wfm_trash_id = resp_dict['tree']['children'][1]['node']['_id']
        except KeyError:
            return False
    ...
    def get(self, doc_id):
        """Downloads the requested document from the server.
        Returns a dictionary containing the type, name, size and
        data of the requested file.
        """
        url = '%s/%s/%s/' % (self._wfm_url, 'get', doc_id)

        try:
            response = self._url_obj.open(url)
        except urllib2.HTTPError, e:
            if e.code == 404:
                return {'success': False, 'status_msg': 'document_does_not_exist'}
            else:
                return {'success': False, 'status_msg': 'server_problem'}

        read = response.read()
        try:
            return json.loads(read)
        except:
            ftype = response.info()['Content-Type']
            fsize = long(response.info()['Content-Length'])
            fname = response.info()['Content-Disposition'].split('filename=')[1].strip('"')
            return {'success': True, 'type': ftype, 'name': fname, 'size': fsize, 'data': read}


    @execute_command
    def cmd_ls(self, doc_id=None, path='root/home', group_id=''):
        """List the documents under the given path, document or group.
        """
        if doc_id is None:
            doc_id = self._wfm_home_id

        return {'group_id': group_id, 'path': path, 'doc_id': doc_id}
    ...
    def cmd_create_file(self, f, parent_id=None):
        ...
        parent_id is not a folder, an error will
        occur. If parent_id is None the file will
    ...
    def cmd_restore(self, doc_id):
        """Restore the target document from trash."""
        return {'doc_id' : doc_id}
    ...
    def cmd_get_thumbnail(self, doc_id):
        """Returns a dict that contains the name of the thumbnail
        and an Image class instance containing the image.
        In case of error, it returns a dict with the corresponding
        error message.
        """
        url = '%s/%s/%s' % (self._wfm_url, 'cmd_get_thumbnail', '?doc_id=' + doc_id)
        ...
        try:
            ...
            image = Image.open(StringIO(read))
            name = 'thumb_' + response.info()['Content-Disposition'].split('filename=')[1]
            return {'success': True, 'name': name, 'image': image}
